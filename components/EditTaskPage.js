...
export default function EditEntry({ route, navigation }) {
  const { entryId, currentDate } = route.params;
  ...
  useEffect(() => {
    const fetchEntry = async () => {
      try {
        ...
        console.log('Existing entries from AsyncStorage:', existingEntries); 
        ...
        if (existingEntries) {
          ...
          entries.forEach(entry => console.log('Checking entry with ID:', entry.id));
    
          const entry = entries.find(item => item.id === entryId);
          console.log('Entry with ID:', entryId, 'is', entry); 
    
          if (entry) {
            setTitle(entry.title);
            setText(entry.text);
            setImageUri(entry.imageUri || null);
            setIsCheckbox(entry.type === 'task');
          } else {
            Alert.alert('Error', 'Entry not found.');
          }
        } else {
          Alert.alert('Error', 'No entries found for this user.');
        }
      } catch (error) {
  ...
  const saveEntry = async () => {
    ...
    try {
      const data = {
        id: entryId, // Keep the same ID for editing
        ...
        completed: isCheckbox ? false : undefined, // Only include if task
      };
      ...
      entries = entries.map(item => (item.id === entryId ? data : item));
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.dateContainer}>
        ...
        <Text style={styles.dateText}>{`Thu ${currentDate}`}</Text>
      </View>
